...
class edge {
    ...
    handleCommPort(args) {
        ...
        let self = this
        try {
            self.serialPort = new SerialPort(self.commPort, { "baudRate": self.baud, autoOpen: false }, function (err) {
                if (err) {
    ...
    compileWrite(message) {
        ...
        bufdata[0] = message.rtuId || 1                         //rtuid
        bufdata[1] = 16                                         //function code
        bufdata[2] = (message.modbusRegister / 256) & 256       //Start Register
        bufdata[3] = message.modbusRegister & 256               //Start Register
    ...
    compileOutputs(args) {
        ...
        for (let i = 0; i < self.commands.length; i++) {
            self.commands[i].output.push(self.commands[i].RTUAddress)
            self.commands[i].output.push(self.commands[i].functionCode)
            self.commands[i].output.push((self.commands[i].register / 256) & 255)
            self.commands[i].output.push(self.commands[i].register & 255)
            self.commands[i].output.push((1 / 256) & 255)   //Register Length
            self.commands[i].output.push(1 & 255)           //Register Length
            let crc = self.calculateCRC(self.commands[i].output)
            self.commands[i].output.push(crc & 255)
            self.commands[i].output.push((crc >> 8) & 255)
        }
        deferred.resolve(args)
        return deferred.promise
    }
